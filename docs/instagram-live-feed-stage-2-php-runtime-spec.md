...
## Implementation notes (тимчасовий блок — видалити після Етапу 4)

> Цей розділ — фіксований звіт після виконання ТЗ у Сесії 25 (2026-05-04).
> Має тимчасовий характер: після того як Етапи 3 і 4 будуть закриті
> (cron + frontend integration + закриття перерахованих ризиків),
> цей блок треба видалити, бо він дублює інформацію з `PROJECT.md` і
> застаріє разом із наступними змінами.
...
3. Інакше — `curl` до `https://graph.facebook.com/{INSTAGRAM_GRAPH_API_VERSION}/{IG_USER_ID}/media?fields=...&limit=3&access_token={PAGE_ACCESS_TOKEN}` з `connect_timeout=3s, timeout=5s`.
...
1. **Cron на PixelX** — потрібен інтерфейс хостингу для налаштування. Команда: `php /home/.../public/api/instagram-refresh-token.php`. Endpoint наразі публічний — на Етапі 3 додати простий guard (секрет у query або `Require ip` у `.htaccess`).
2. **`expires_at=null` після refresh** — Meta для page tokens, отриманих з long-lived user token, не повертає `expires_in` у `/me/accounts`. На Етапі 3 додатково викликати `/debug_token` після рефреша й писати реальний `data_access_expires_at`.
3. **Версія API** — винесена як `INSTAGRAM_GRAPH_API_VERSION` у конфіг, оновлюється без зміни коду.
4. **Frontend (Етап 4)** — Vite dev `:8080` PHP не виконає. Або підняти PHP-server збоку (`php -S 127.0.0.1:8765 -t public`) і додати `server.proxy['/api']` у `vite.config.ts`, або тестувати фронт лише на хості після першого деплою.
5. **DSGVO / Consent (Етап 5)** — `imageUrl` веде на `*.cdninstagram.com`, тож обов'язково під External Content gate.
